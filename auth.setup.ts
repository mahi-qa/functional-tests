...
setup("authenticate", async ({ page }) => {
  ...
  console.log("Starting Setup - Tutor Login");
  ...
  try {
    ...
    await page
      ...
      .fill(configs.TEST_CONSTANTS.TEST_TUTOR_EMAIL);
    ...
    await page
      ...
      .fill(configs.TEST_CONSTANTS.TEST_TUTOR_PASSWORD);
    await page.getByRole("button", { name: "Next" }).click();
...
async function handle2FA(page) {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      const token = generateToken(configs.TEST_CONSTANTS.TEST_TUTOR_KEY);
